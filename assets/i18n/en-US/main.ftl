...
settings-lastfm-off = Not connected
settings-lastfm-waiting = Waiting for Last.fm in your browser…
settings-lastfm-as = Scrobbling as { $name }
settings-lastfm-failed = Could not connect to Last.fm
settings-lastfm-connect = Connect
settings-lastfm-disconnect = Disconnect
settings-lastfm-title = Connect Last.fm
settings-lastfm-title-detail = Sonora scrobbles through your own Last.fm API account. Create one, then paste the key and the secret here.
settings-lastfm-key = API key
settings-lastfm-secret = API secret
settings-lastfm-request = Get an API key
